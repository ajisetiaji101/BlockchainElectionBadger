...
// func (p2p *P2PNetwork) Add Block API(voterID, candidateID string) {
func (p2p *P2PNetwork) AddRiwayatTrx(data model.AddDataRiwayat) {
	// for _, block := range p2p.Blockchain.Blocks {
	// 	if block.Data.UniqueCodePayment == uniqueCodePayment && block.Data.Nik == nik {
	// 		fmt.Println("Unique Code Payment already recored:", uniqueCodePayment)
	// 		return
	// 	}
	// }

	transactionData := blockchain.DataRiwayat{
		UniqueCodePayment: data.UniqueCodePayment,
		Amount:            data.Amount,
		Tax:               data.Tax,
		WarungCode:        data.WarungCode,
		Payment:           data.Payment,
		Date:              data.Date,
		Nik:               data.Nik,
		NikCustomer:       data.NikCustomer,
		Timestamp:         time.Now().Unix(),
	}

	//convert transactionData to string kemudian ubah lagi ke semula
	transactionDataString, err := sonic.Marshal(transactionData)
	if err != nil {
		fmt.Println("Error marshalling transaction data:", err)
		return
	}

	// ubah dari YYYY-MM-DD ke YYYYMMDD
	date, err := time.Parse("2006-01-02", data.Date)

	blockchain.Handle(err)

	key := fmt.Sprintf("%s_%s_%s", data.WarungCode, data.UniqueCodePayment, date.Format("20060102"))
	...
	// 	Timestamp: time.Now().Unix(),
	// 	Data:      []byte(transactionDataString),
	// 	PrevHash:  []byte{},
	// }
...
// func (p2p *P2PNetwork) HandleVote(uniqueCodePayment string, amount int, tax string, warungCode string, payment string, date string, nik string, nikCustomer string) {
// 	for _, block := range p2p.Blockchain.Blocks {
// 		if block.Data.UniqueCodePayment == uniqueCodePayment && block.Data.Nik == nik {
// 			fmt.Println("Unique Code Payment already recored:", uniqueCodePayment)
// 			return
...
// 	transactionData := blockchain.DataRiwayat{
// 		UniqueCodePayment: uniqueCodePayment,
// 		Amount:            amount,
// 		Tax:               tax,
// 		WarungCode:        warungCode,
// 		Payment:           payment,
// 		Date:              date,
// 		Nik:               nik,
...
// 			err := p2p.Blockchain.Election.Vote(uniqueCodePayment, warungCode) // Pastikan Election ada di blockchain
// 			if err != nil {
// 				fmt.Println("Error while voting:", err)
// 			} else {
// 				p2p.BroadcastBlockchain()
// 				fmt.Println("History successful for record:", warungCode, "with unique code payment:", uniqueCodePayment)
// 			}
// 		}
// 	} else {
