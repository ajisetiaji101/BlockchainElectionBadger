...
func NewP2PNetwork(bootstrap, localAddr string, privateKey *ecdsa.PrivateKey, publicKey []byte, dbConn *badger.DB) *P2PNetwork {
	return &P2PNetwork{
		peers:      make([]Peer, 0),
		Blockchain: &blockchain.Blockchain{},
		bootstrap:  bootstrap,
...
func (p2p *P2PNetwork) GetConnectionDB() (*badger.DB, error) {
	if p2p.dbConn == nil {
		return nil, fmt.Errorf("database connection is nil")
	}
	return p2p.dbConn, nil
}
